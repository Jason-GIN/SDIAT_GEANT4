...
SphericalSystemSDinAir::SphericalSystemSDinAir(const G4String& name,
                                               const std::vector<G4double>& radii)
    : G4VSensitiveDetector(name), fRadii(radii)
{
    collectionName.insert("SystemHitsinAirCollection");
    ...
    */
    ...
    G4cout << "SphericalSystemSD constructor: " << "SystemHitsinAirCollection" << G4endl;
}

void SphericalSystemSDinAir::Initialize(G4HCofThisEvent* hce)
{
  ...
  fSystemHitsinAirCollection
    = new SphericalHitsinAirCollection(SensitiveDetectorName, collectionName[0]);
  ...
  hce->AddHitsCollection( hcID, fSystemHitsinAirCollection );
  //G4cout << "Spherical Hits collection created with ID: " << hcID << G4endl;
  
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4bool SphericalSystemSDinAir::AltJudge(G4ThreeVector xyz, G4double steplength){
  G4double Alt = xyz.mag();
  G4double Range = Alt / (1.0*km) - std::floor(Alt / (1.0*km));
  if ( /*Range*km == steplength ||*/ Range*km < steplength 
    || Range*km > (1.0*km-steplength) /*|| Range*km == (1.0*km-steplength)*/){
    return true;
  }else{
    return false;
  }
...
G4bool SphericalSystemSDinAir::ProcessHits(G4Step* aStep,
                                     G4TouchableHistory*)
{
  if (!fSystemHitsinAirCollection) {
  ...
  auto newHit = new SphericalHitinAir();
  ...
  fSystemHitsinAirCollection->insert( newHit );
  ...
  G4int SDnum =newHit->GetChamberNb();
  //You should consider the copy number is not always equal to the height, 
  //because of the different set of air layer.
  //if (SDnum > 99 ){
  //  SDnum=SDnum+5;
  //}

  G4String Detector="Air";

  G4String Process = newHit->GetProcess();
  //G4double stepL = aStep->GetStepLength();
  //G4cout << "-------------GIN3SSD: stepL: "<< stepL/km <<"---------------" << G4endl;
  //ParentId >= 1 &&
  if ( /*&& PID == 22*/ kE>=30.*MeV /*&& AltJudge(position,stepL)*/){
    //newHit->Print();
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void SphericalSystemSDinAir::EndOfEvent(G4HCofThisEvent*)
{
  if ( verboseLevel>1 ) {
     G4int nofHits = fSystemHitsinAirCollection->entries();
     G4cout << G4endl
